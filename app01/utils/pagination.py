...
class Pagination:
    ...
    def page_html(self):
        ...
        li = f'<li class="page_first"><a href="{self.base_url}?{self.query_encode}#{self.position}">首页</a></li>'
        page_list.append(li)

        # 生成分页上一页
        if self.current_page != 1:
            self.query_params['page'] = self.current_page - 1
            li = f'<li class="page_up"><a href="{self.base_url}?{self.query_encode}#{self.position}">上一页</a></li>'
            page_list.append(li)
        ...
        if self.current_page != self.current_count:
            self.query_params['page'] = self.current_page + 1
            li = f'<li class="page_down"><a href="{self.base_url}?{self.query_encode}#{self.position}">下一页</a></li>'
            page_list.append(li)

        # 生成页尾页
        self.query_params['page'] = self.current_count
        li = f'<li class="page_last"><a href="{self.base_url}?{self.query_encode}#{self.position}">尾页</a></li>'
        page_list.append(li)
        # print(page_list)
    ...
    def query_encode(self):
        ...
        return self.query_params.urlencode()  # 后端获取的是queryset, 自带这个方法
...
class TestPagination(Pagination):
    def page_html(self):
        ...
        # 生成页首页
        self.query_params['page'] = 1
        li = f'<li class="page_first"><a href="{self.base_url}?{self.query_encode}#{self.position}">首题</a></li>'
        page_list.append(li)

        # 生成分页上一页
        if self.current_page != 1:
            self.query_params['page'] = self.current_page - 1
            li = f'<li class="page_up"><a href="{self.base_url}?{self.query_encode}#{self.position}">上一题</a></li>'
            page_list.append(li)
        ...
        if self.current_page != self.current_count:
            self.query_params['page'] = self.current_page + 1
            li = f'<li class="page_down"><a href="{self.base_url}?{self.query_encode}#{self.position}">下一题</a></li>'
            page_list.append(li)

        # 生成页尾页
        self.query_params['page'] = self.current_count
        li = f'<li class="page_last"><a href="{self.base_url}?{self.query_encode}#{self.position}">尾题</a></li>'
        page_list.append(li)
        # print(page_list)
        return ''.join(page_list)
