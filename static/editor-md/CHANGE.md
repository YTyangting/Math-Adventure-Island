...
- 合并 CodeMirror 使用到的多个 JS 模块文件，大幅减少 HTTP 请求，加快下载速度；
    - 新增合并后的两个模块文件：`./lib/codemirror/modes.axios.min.js`、`./lib/codemirror/addons.axios.min.js` ；
...
- 改进 [#28](https://github.com/pandao/editor.md/issues/28)；
    - 将 `jquery.axios.min.js` 、 `font-awesome.min.css` 、 `github-markdown.css` 移除（这是一个疏忽，它们不是动态加载的依赖模块或者不需要的，避免不必要的硬盘空间占用）；
